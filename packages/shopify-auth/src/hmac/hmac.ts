...
export type VerifyFn = (input: VerifyInput) => ShopOrigin;
...
export class InvalidHmacError extends Error {
  ...
  constructor(hmac: string, query: string) {
...
function verify(config: VerifyConfig): VerifyFn {
  ...
  return (input) => {
    const hmac = searchParams.get(input.searchParams, config.key);
    const _shopOrigin = parseShopOrigin(input.searchParams);

    const _isValidPayload = isValidPayload({
      hmac,
      payload: _encodeSearchParams(input.searchParams).join(config.separator),
      secret: input.apiSecret,
    });

    if (!_isValidPayload) {
      throw new InvalidHmacError(hmac, input.searchParams.toString());
    }
