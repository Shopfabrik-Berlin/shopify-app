...
export function getAccessToken(env: GetAccessTokenEnv) {
  return async (input: GetAccessTokenInput): Promise<GetAccessTokenPayload> => {
    ...
    if (!response.ok) {
      throw new FetchError('Failed fetching shopify access token', response);
    }
